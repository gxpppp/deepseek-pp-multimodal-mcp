...
async function callTool(params, env) {
  ...
  try {
    ...
    if (name === 'analyze_images') return successToolResult(await analyzeImages(args, env));
    if (name === 'analyze_video') return successToolResult(await analyzeVideo(args, env));
    if (name === 'analyze_images_siliconflow') return successToolResult(await analyzeImagesSiliconFlow(args, env));
    if (name === 'analyze_video_siliconflow') return successToolResult(await analyzeVideoSiliconFlow(args, env));
...
async function visionStatus(env) {
  ...
  return {
    ...
    siliconflow: {
      configured: Boolean(getSiliconFlowKey(env)),
...
function normalizeSiliconFlowImages(rawImages, env) {
  ...
  return rawImages.map((raw, index) => {
    ...
    if (localPath) {
      ...
      const bytes = readFileSync(localPath);
      if (bytes.length > maxDataUrlBytes) {
        throw validationError(`images[${index}].localPath exceeds ${maxDataUrlBytes} bytes. Use an HTTP URL instead.`);
      }
      url = `data:${mimeType};base64,${bytes.toString('base64')}`;
    } else if (nonEmptyString(imageUrlObj.url)) {
      url = imageUrlObj.url;
...
function normalizeSiliconFlowVideo(rawVideo, env) {
  ...
  if (localPath) {
    ...
    if (bytes.length > maxInlineVideoBytes) {
      throw validationError(`video.localPath exceeds ${maxInlineVideoBytes} bytes. Use an HTTP URL instead.`);
    }
    resolvedUrl = `data:${mimeType};base64,${bytes.toString('base64')}`;
    source = 'localPath';
